...
# ---------------------------------------------------------------------------
async def wait_for_event(task_id: UUID, event_type: str, timeout: float = 600) -> bool:
    """Block until an event for the given task appears on the task_events stream."""
    rd = await get_redis()
    # Create a unique consumer group for this test run
    group = f"test-wait-{uuid4()}"
    try:
        await rd.xgroup_create(STREAM_TASK_EVENTS, group, id="$", mkstream=True)
    except Exception:
        pass   # group may already exist – rare with unique name
    ...
    while time.time() - start < timeout:
        ...
        for stream_name, msg_list in msgs:
            for msg_id, fields in msg_list: # type: ignore
                if fields.get("task_id") == str(task_id) and fields.get("event") == event_type:
                    await rd.xack(STREAM_TASK_EVENTS, group, msg_id)
                    return True
                # Acknowledge other messages so they don't pile up
                await rd.xack(STREAM_TASK_EVENTS, group, msg_id)
        # Small sleep to avoid tight loop if no messages
...
async def main(queries: list[str]):
    ...
    # 3.2 Wait for the final report event for each task
    for tid in task_ids:
        print(f"\nWaiting for report on task {tid} ...")
        ok = await wait_for_event(tid, "report_completed", timeout=600)
        if not ok:
            print(f"ERROR: Timeout waiting for report on task {tid}")
            continue

        # 3.3 Fetch and print everything now that the pipeline is complete
        async with async_session() as session:
            task = await session.get(ResearchTask, tid)
            if not task:
            ...
            # ---- Critiques ----
            crit_stmt = select(Critique).join(Claim, Critique.claim_id == Claim.id).where(Claim.task_id == tid)
            critiques = (await session.execute(crit_stmt)).scalars().all()
            if critiques:
                print(f"\nCritiques for task: {task.query}")
                for i, critique in enumerate(critiques, 1):
                    print(f"  Critique {i}:")
                    print(f"    Critic: {critique.critic_name}")
                    print(f"    Text: {critique.critique_text[:200]}...")
            ...
            # ---- Report + PDF ----
            report_stmt = select(ResearchReport).where(ResearchReport.task_id == tid)
...
{html_body}
                ...
                try:
                    safe_name = task.query.replace(' ', '_').replace('/', '_')[:50]
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        queries = ["What Happens Due to O3 Deficiency in Humans ?"]
    asyncio.run(main(queries=queries))